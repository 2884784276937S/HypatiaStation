...
/obj/machinery/newscaster/attackby(I as obj, user as mob)
	if(istype(I, /obj/item/weapon/screwdriver))
		...
		if(do_after(user, 20))
			var/obj/machinery/newscaster/A = new /obj/machinery/newscaster( src.loc )
			A.anchored = 1
			...
			if (src.stat & BROKEN)
				...
				A.icon_state = "newscaster_noglass"
			del(src)
	else
...
/obj/machinery/newscaster/New()         //Constructor, ho~
	...
	component_parts += new /obj/item/weapon/circuitboard/news(src)
...
/obj/machinery/newscaster/Topic(href, href_list)
	...
	if ((usr.contents.Find(src) || ((get_dist(src, usr) <= 1) && istype(src.loc, /turf))) || (istype(usr, /mob/living/silicon)))
		...
		if(href_list["set_channel_name"])
			...
			src.msg = replacetext(src.msg, "\[/center\]", "</center>")
			src.msg = replacetext(src.msg, "\[br\]", "<BR>")
			src.msg = replacetext(src.msg, "\[b\]", "<B>")
			src.msg = replacetext(src.msg, "\[/b\]", "</B>")
			src.msg = replacetext(src.msg, "\[i\]", "<I>")
			src.msg = replacetext(src.msg, "\[/i\]", "</I>")
