 ...
 */
 ...
 //This now uses the round_persist database, as I'm trying to phase all logs, configuration type things, admin stuff and ban stuff, and polls in feedback,
 //trying to completely phase out tgstation database, and I want all round persistant stuff (including stuff I'm working on) to go in round_persist.
 //-Numbers
	...
	switch(screenstate)
		...
		if(1)
			...
			if(!dbcon_round.IsConnected())
				...
				var/DBQuery/query = dbcon_round.NewQuery(SQLquery)
				query.Execute()
	...
	switch(screenstate)
		...
		if(4)
			...
			if(!dbcon_round.IsConnected())
				...
				var/DBQuery/query = dbcon_round.NewQuery("SELECT id, author, title, category FROM library")
				query.Execute()
...
/obj/machinery/librarycomp/Topic(href, href_list)
				...
				if(choice == "Confirm")
					...
					if(!dbcon_round.IsConnected())
						...
						*/
						...
						var/DBQuery/query = dbcon_round.NewQuery("INSERT INTO library (author, title, content, category) VALUES ('[sqlauthor]', '[sqltitle]', '[sqlcontent]', '[sqlcategory]')")
						if(!query.Execute())
							usr << query.ErrorMsg()
						else
	...
	if(href_list["targetid"])
		...
		if(bibledelay)
			...
			spawn(60)
				bibledelay = 0
			var/DBQuery/query = dbcon_round.NewQuery("SELECT * FROM library WHERE id=[sqlid]")
			query.Execute()
