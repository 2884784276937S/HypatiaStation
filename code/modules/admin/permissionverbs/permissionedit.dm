...
/datum/admins/proc/log_admin_rank_modification(var/adm_ckey, var/new_rank)
	...
	var/DBQuery/select_query = dbcon.NewQuery("SELECT id FROM admin WHERE ckey = '[adm_ckey]'")
	...
	if(new_admin)
		var/DBQuery/insert_query = dbcon.NewQuery("INSERT INTO `admin` (`id`, `ckey`, `rank`, `level`, `flags`) VALUES (null, '[adm_ckey]', '[new_rank]', -1, 0)")
		...
		if(!isnull(admin_id) && isnum(admin_id))
			var/DBQuery/insert_query = dbcon.NewQuery("UPDATE `admin` SET rank = '[new_rank]' WHERE id = [admin_id]")
			insert_query.Execute()
...
/datum/admins/proc/log_admin_permission_modification(var/adm_ckey, var/new_permission)
	...
	var/DBQuery/select_query = dbcon.NewQuery("SELECT id, flags FROM admin WHERE ckey = '[adm_ckey]'")
	select_query.Execute()
	...
	if(admin_rights & new_permission) //This admin already has this permission, so we are removing it.
		var/DBQuery/insert_query = dbcon.NewQuery("UPDATE `admin` SET flags = [admin_rights & ~new_permission] WHERE id = [admin_id]")
		insert_query.Execute()
