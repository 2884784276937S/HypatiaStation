/datum/admins/Topic(href, href_list)
	...
	if(href_list["makeAntag"])
		...
		switch(href_list["simplemake"])
			...
			if("hk")			M.change_mob_type( /mob/living/simple_animal/cat/HappyKitten , null, null, delmob )
			if("kitten")			M.change_mob_type( /mob/living/simple_animal/cat/kitten , null, null, delmob )
...
*/
	...
	//JOBBAN'S INNARDS
		...
		if(joblist.len) //at least 1 banned job exists in joblist so we have stuff to unban.
			if(!config.ban_legacy_system)
				usr << "Unfortunately, database based unbanning cannot be done through this panel, switching to DB ban panel."
				DB_ban_panel(M.ckey)
				return
			var/msg
			for(var/job in joblist)
