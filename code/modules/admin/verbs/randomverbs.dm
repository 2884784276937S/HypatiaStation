...
/client/proc/admin_call_transfer()
	...
	emergency_shuttle.incall()
	captain_announce("A crew transfer has been initiated. The shuttle has been dispatched. It will arrive in [round(emergency_shuttle.timeleft()/60)] minutes.")
	world << sound('sound/AI/shuttlecalled.ogg')
	feedback_add_details("admin_verb","TRANSHUT") //If you are copy-pasting this, ensure the 2nd parameter is unique to the new proc!
	log_admin("[key_name(usr)] admin-called the transfer shuttle.")
	message_admins("\blue [key_name_admin(usr)] admin-called the transfer shuttle.", 1)
	return
