/mob/living/carbon/roro/Life()
	...
	if(stat != DEAD)
		...
		//Disease Check
		//handle_virus_updates() There is no disease that affects roros
...
/mob/living/carbon/roro
	...
	var/Discipline = 0 // if a roro has been hit with a freeze gun, or wrestled/attacked off a human, they become disciplined and don't attack anymore for a while
	...
	proc
		...
		AIprocess()  // the master AI process
			...
			while(AIproc && stat != 2 && (attacked > 0 || starving || hungry || rabid || Victim))
				...
				if(Target)
					for(var/mob/living/carbon/roro/M in view(1,Target))
						if(M.Victim == Target)
							Target = null
							AIproc = 0
							break
				...
				sleep(sleeptime + 2) // this is about as fast as a player roro can go
		...
		handle_nutrition()
			...
			if(prob(20))
				if(istype(src, /mob/living/carbon/roro/adult)) nutrition-=rand(4,6)
				...
				if(istype(src, /mob/living/carbon/roro/adult))
					if(!client)
						...
						for(var/i=1,i<=number,i++) // reproduce (has a small chance of producing 3 or 4 offspring)
							var/mob/living/carbon/roro/M = new/mob/living/carbon/roro(loc)
//							M.nutrition = round(nutrition * 0.9)
					...
					if(!client)
						var/mob/living/carbon/roro/adult/A = new/mob/living/carbon/roro/adult(src.loc)
...
//						A.nutrition += 100
		...
		handle_targets()
			...
			if(!client)
				...
				var/hungry = 0 // determines if the roro is hungry
				var/starving = 0 // determines if the roro is starving-hungry
				if(istype(src, /mob/living/carbon/roro/adult)) // 1200 max nutrition
					switch(nutrition)
						if(601 to 900)
				...
				if(starving && !client) // if a roro is starving, it starts losing its friends
				...
				if(!Target)
					...
					if(hungry || starving) //Only add to the list if we need to
						for(var/mob/living/L in view(7,src))
							...
							//Ignore other roros, dead mobs and simple_animals
							if(isslime(L) || L.stat == DEAD || isanimal(L))
								continue

							if(issilicon(L))
								if(!istype(src, /mob/living/carbon/roro/adult)) //Non-starving diciplined adult roros wont eat things
									if(!starving && Discipline > 0)
										continue

								if(tame) //Tame roros ignore electronic life
									continue
								...
								if(istype(L, /mob/living/carbon/human)) //Ignore roro(wo)men
									var/mob/living/carbon/human/H = L
									if(H.dna)
										if(H.dna.mutantrace == "roro")
											continue
								...
								if(tame && ishuman(L)) //Tame roros dont eat people.
									continue

								if(!L.canmove) //Only one roro can latch on at a time.

									var/notarget = 0
									for(var/mob/living/carbon/roro/M in view(1,L))
										if(M.Victim == L)
											notarget = 1
									if(notarget)
										continue
