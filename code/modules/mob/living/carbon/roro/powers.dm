/mob/living/carbon/roro/verb/Feed()
	set category = "Roro"
	...
	for(var/mob/living/C in view(1,src))
		if(C!=src && !istype(C,/mob/living/carbon/roro))
			choices += C
	...
	if(M in view(1, src))
		...
		if(!istype(src, /mob/living/carbon/brain))
			if(!istype(M, /mob/living/carbon/roro))
				if(stat != 2)
					if(health > -70)

						for(var/mob/living/carbon/roro/met in view())
...
/mob/living/carbon/roro/proc/Feedon(var/mob/living/carbon/M)
	Victim = M
	src.loc = M.loc
	canmove = 0
	anchored = 1
	...
	while(Victim && M.health > -70 && stat != 2)
		...
		if(M in view(1, src))
			...
			if(istype(M, /mob/living/carbon))
				...
				if(Victim)
					for(var/mob/living/carbon/roro/roro in view(1,M))
						if(roro.Victim == M && roro != src)
							roro.Feedstop()
...
/mob/living/carbon/roro/proc/Feedstop()
	if(Victim)
		if(Victim.client) Victim << "[src] has let go of your head!"
		Victim = null

/mob/living/carbon/roro/proc/UpdateFeed(var/mob/M)
	if(Victim)
		if(Victim == M)
			loc = M.loc // simple "attach to head" effect!


/mob/living/carbon/roro/verb/Evolve()
	set category = "Roro"
	set desc = "This will let you evolve from baby to adult roro."
	...
	if(!istype(src, /mob/living/carbon/roro/adult))
		if(amount_grown >= 10)
			var/mob/living/carbon/roro/adult/new_roro = new /mob/living/carbon/roro/adult(loc)
			new_roro.nutrition = nutrition
			new_roro.powerlevel = max(0, powerlevel-1)
			new_roro.a_intent = "hurt"
			new_roro.key = key

			new_roro << "<B>You are now an adult roro.</B>"
			del(src)
		else
...
/mob/living/carbon/roro/verb/Reproduce()
	set category = "Roro"
	set desc = "This will make you split into a random number of roros (usually 2). NOTE: this will KILL you, but you will be transferred into one of the babies."
	...
	if(istype(src, /mob/living/carbon/roro/adult))
		if(amount_grown >= 10)
			if(input("Are you absolutely sure you want to reproduce? Your current body will cease to be, but your consciousness will be transferred into a produced rorobeast.") in list("Yes","No")=="Yes")
				...
				var/mob/living/carbon/roro/new_roro = pick(babies)
				new_roro.a_intent = "hurt"
				new_roro.key = key

				new_roro << "<B>You are now a Rorobeast. Skree!</B>"
				del(src)
		else
...
/mob/living/carbon/roro/verb/ventcrawl()
