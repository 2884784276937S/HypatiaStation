	...
	Bump(atom/A as mob|obj|turf|area)
		...
		if(firer && istype(A, /mob))
			...
			if(istype(firer, /mob))
				...
				msg_admin_attack("ATTACK: [firer] ([firer.ckey]) shot [M] ([M.ckey]) with a [src]",firer,M) //BS12 EDIT ALG
			else
				M.attack_log += "\[[time_stamp()]\] <b>UNKNOWN SUBJECT (No longer exists)</b> shot <b>[M]/[M.ckey]</b> with a <b>[src]</b>"
				log_attack("<font color='red'>UNKNOWN shot [M] ([M.ckey]) with a [src.type]</font>")
				msg_admin_attack("ATTACK: UNKNOWN shot [M] ([M.ckey]) with a [src]",firer,M) //BS12 EDIT ALG
