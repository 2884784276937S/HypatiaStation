...
/datum/event/economic_event/announce()
	...
	switch(event_type)
		...
		if(PIRATES)
			newMsg.body = "[pick("Pirates","Criminal elements","A [pick("Syndicate","Eidola Pirates","\'REDACTED\'")] strike force")] have [pick("raided","blockaded","attempted to blackmail","attacked")] [affected_dest.name] today. Security has been tightened, but many valuable minerals were taken."
		if(CORPORATE_ATTACK)
