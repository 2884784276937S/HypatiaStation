...
/datum/event/economic_event/announce()
	...
	switch(event_type)
		...
		if(PIRATES)
			newMsg.body = "[pick("Pirates","Criminal elements","A [pick("Eidola Pirates","\'REDACTED\'")] strike force")] have [pick("raided","blockaded","attempted to blackmail","attacked")] [affected_dest.name] today. Security has been tightened, but many valuable minerals were taken."
		if(CORPORATE_ATTACK)
			newMsg.body = "A small [pick("pirate","Cybersun Industries","Gorlex Marauders")] fleet has precise-jumped into proximity with [affected_dest.name], [pick("for a smash-and-grab operation","in a hit and run attack","in an overt display of hostilities")]. Much damage was done, and security has been tightened since the incident."
//		if(ALIEN_RAIDERS)
//			if(prob(20))
...
//				newMsg.body = "[pick("The alien species designated \'United Exolitics\'","The alien species designated \'REDACTED\'","An unknown alien species")] have raided [affected_dest.name] today, stealing wildlife, farm animals, medical research materials and kidnapping civilians. It seems they desire to learn more about us, so the Navy will be standing by to accomodate them next time they try."
		if(ORION_BLADES)
			newMsg.body ="The Orion Blades have raided [affected_dest.name] today, inflicting [pick("Massive","Major","Minor","Minimal")] damage on the local infrastructure. NanoTrasen authorities are standing by to counter further attacks."
		if(AI_LIBERATION)
